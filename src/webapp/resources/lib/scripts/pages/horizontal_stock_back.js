...
$("#btnExport").on("click", function () {
    ...
    $.SubAjax({
        ...
        url: "/DataImport/Stock/ExportHorizontalStock",
        success: function (data) {
            if (data.IsSuccess) {
                window.location.href = "/File/DownFile?filePath=" + data.Data + "&fileName=历史库存列表横版";
            }
            else {
...
function LoadDatatable() {
                        ...
                        columns.push({
                            ...
                            formatter: function (value, row, index) {
                                ...
                                if (value != "") {
                                    ...
                                    if (maxD != -1) {
                                        if (k >= maxD) {
                                            itemResult = "<strong class='red'>" + v + "</strong>";
                                        }
                                    }
                                    if (minD != -1) {
                                        if (k <= minD) {
                        ...
                        columns.push({
                            ...
                            formatter: function (value, row, index) {
                                ...
                                if (value != "") {
                                    ...
                                    var minD = $("#MinStockWarning").val();
                                    if (maxD != -1) {
                                        var g = parseFloat(maxD);
                                        if (k >= g) {
                                            itemResult = "<strong class='red'>" + v + "</strong>";
                                        }
                                    }
                                    if (minD != -1) {
                                        var h = parseFloat(minD);
                                        if (k <= h) {
                                            itemResult = "<strong class='red'>" + v + "</strong>";
                                        }
                                    }
    ...
    $('#dataTable').bootstrapTable({
        url: "/DataImport/Stock/AllStockReport",
        pagination: "true",
