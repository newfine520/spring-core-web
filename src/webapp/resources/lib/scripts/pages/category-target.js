$(function () {
    ...
    $('#dataTable').bootstrapTable({
        url: "/DataImport/CategoryTarget/QueryCategoryTarget",
        pagination: "true",
    ...
    $('#modalImport').on('shown.bs.modal', function (e) {
        uploader = WebUploader.create({
            ...
            // 文件接收服务端。
            server: '/DataImport/CategoryTarget/ImportTarget',
            // 选择文件的按钮。可选。
    ...
    $.SubAjax({
        ...
        success: function (data) {
            if (data.IsSuccess) {
                ...
                if(typeTarget=="001"||typeTarget=="002")
                    $("#btnImport").hide();
            }
            else {
...
$("#btnExport").on("click", function () {
    $.SubAjax({
        ...
        url: "/DataImport/CategoryTarget/ExportData",
        success: function (data) {
            if (data.IsSuccess) {
                window.location.href = "/File/DownFile?filePath=" + data.Data + "&fileName=品类指标";
            }
            else {
                $.ShowMessage("error", data.Msg)
            }
