...
function locationFormatter(value, row, index) {
    if (row.StoreLocation != null && row.StoreLocation != "") { var results = '<a class="location"  title="门店定位"><image src="/images/location.jpg" class="loc"/></a>'; }
    return results;
}
...
var StoreTypeForm = $("#StoreTypeForm");
StoreTypeForm.validate({
    rules: {
        StoreTypeNo: {
            required: true
        },
...
$("#btnAdd").on("click", function () {
    var storeType = $("#selPositionStatusCode").val();
    if (StoreForm.valid()) {
        $.SubAjax({
            type: 'post',
            url: '/System/StoreType/AddStoreType?StoreType='+storeType,
            data: StoreTypeForm.serializeToJson(),
...
$("#btnUpdate").on("click", function () {
    ...
    if (StoreForm.valid()) {
        $.SubAjax({
            type: 'post',
            url: '/System/Store/UpdateStore?PromoterId=' + promoterId + '&SupervisorId=' + supervisorId,
    ...
    $.SubAjax({
        ...
        success: function (data) {
            if (data.IsSuccess) {
                ...
                $("#StoreActivationStatus").val(data.StoreActivationStatus);
            }
            else {
                $.ShowMessage("error", data.Msg)
            }
