...
function loadSuveryStore(loaduUrl, data) {
    ...
    $.get(loaduUrl, data, function (data) {
        ...
        var storeSuveryData = [{
            value: ofstore, name: '无货门店数:' + ofstore, label: {
                normal: {
                    show: true,
                    formatter: function (itm) {
                        ...
                        return ['{b|' + changenum + "%" + '}', '无货门店占比'].join('\n');
                    },
                    textStyle: {
                        rich: {
                            a: {
        ...
        var storeOptions = {
            ...
            legend: {
                orient: 'horizontal',
                bottom: 10,
                data: ['无货门店数:' + ofstore, '所辖门店数:' + totalStore],
                itemHeight: 7,
                itemWidth:7,
        ...
        series: [
            {
                ...
                data: [
                    {
                        value: outofSku, name: '无货sku数:' + outofSku,
                        label: {
                            normal: {
                                show: true,
                                formatter: function (itm) {
                                    return ['{b|' + itm.percent.toFixed(1) + "%" + '}', '无货sku占比'].join('\n');
                                },
                                textStyle: {
                                    rich: {
                                        a: {
